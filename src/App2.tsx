...
import { pullCloudRecords, signUpCloudAccount, syncCloudRecords, uploadCloudRecords, withCloudDeletionTombstones } from './domain/cloudSync';
...
const mobileModules = modules.filter((item) => ['home', 'notes', 'privateNotes', 'ledger', 'tasks'].includes(item.key));
...
export function App() {
  ...
  const [cloudRuntime, setCloudRuntime] = useState({ password: '', passphrase: '' });
  ...
  const hasCloudRuntime = Boolean(
    ...
    state.cloudSyncEmail &&
    cloudRuntime.password &&
    cloudRuntime.passphrase
  );

  async function runCloudRecordSync(silent = false) {
    if (!hasCloudRuntime || cloudSyncBusy.current) return;
    cloudSyncBusy.current = true;
    try {
      const result = await syncCloudRecords({
        url: stateRef.current.cloudSyncUrl ?? '',
        publishableKey: stateRef.current.cloudSyncPublishableKey ?? '',
        email: stateRef.current.cloudSyncEmail ?? '',
        password: cloudRuntime.password,
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void runCloudRecordSync(true), 30000);
    void runCloudRecordSync(true);
    return () => window.clearInterval(timer);
  }, [desktopDataLoaded, hasCloudRuntime, cloudRuntime.password, cloudRuntime.passphrase, state.cloudSyncUrl, state.cloudSyncPublishableKey, state.cloudSyncEmail]);
  ...
  useEffect(() => {
    ...
    return () => window.clearInterval(timer);
  }, [notifiedReminderIds, state.reminders, state.ringtone]);
    ...
    <div className={`app-shell ${themeClass} ${mobileApp ? 'mobile-app-shell' : ''}`} style={buildThemeStyle(state)}>
      <header className="topbar">
        <div className="brand-stack">
          ...
          <div className="brand-version">version:0.2.2</div>
        </div>
      ...
      <main className="layout">
        ...
        <section className="workspace">
          ...
          {!mobileApp && activeModule === 'calendar' && <CalendarPanel state={state} setState={setState} targetDate={calendarTargetDate} />}
          {activeModule === 'settings' && <SettingsPanelV2 state={state} setState={setState} cloudRuntime={cloudRuntime} setCloudRuntime={setCloudRuntime} runCloudRecordSync={runCloudRecordSync} mobileApp={mobileApp} />}
...
function HomePanel({ state, setState, setActiveModule, openCalendarDate }: StatePanelProps & { setActiveModule: (module: ModuleKey) => void; openCalendarDate: (date: string) => void }) {
  ...
  const defaultDashboardOrder: DashboardCardId[] = ['summary', 'todos', 'reminders', 'month'];
...
function NotesPanel({ state, setState }: StatePanelProps) {
  const [selectedId, setSelectedId] = useState<string | undefined>(state.notes[0]?.id);
  ...
  const selected = state.notes.find((note) => note.id === selectedId) ?? filteredNotes[0] ?? state.notes[0];
  ...
  return (
    <SplitPanel
      ...
      list={<div className="module-list-surface"><div className="note-filter-row with-export"><label className="module-search"><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="搜索记事关键词" /></label><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /><ToolbarButton onClick={() => void exportCheckedNotes()}><Download size={16} />导出</ToolbarButton></div><label className="checkline select-all-line"><input type="checkbox" checked={filteredNotes.length > 0 && filteredNotes.every((note) => checkedIds.includes(note.id))} onChange={(event) => setCheckedIds(event.target.checked ? filteredNotes.map((note) => note.id) : [])} />全选</label><div className="quick-filter-row"><button className="toolbar-button" onClick={() => applyQuickRange('thisWeek')}>本周</button><button className="toolbar-button" onClick={() => applyQuickRange('lastWeek')}>上周</button><button className="toolbar-button" onClick={() => applyQuickRange('thisMonth')}>本月</button><button className="toolbar-button" onClick={() => applyQuickRange('lastMonth')}>上月</button><button className="toolbar-button" onClick={() => applyQuickRange('thisYear')}>本年度</button><button className="toolbar-button" onClick={() => { setQuery(''); setFrom(''); setTo(''); }}>全部</button></div>{view === 'list' ? <DatedList view={view} items={filteredNotes} selectedId={selected?.id} labelOf={(note) => note.body || '空记事'} leadingOf={(note) => <input type="checkbox" checked={checkedIds.includes(note.id)} onClick={(event) => event.stopPropagation()} onChange={(event) => toggleChecked(note.id, event.target.checked)} aria-label="选择记事" />} onSelect={setSelectedId} contextActions={(note) => [
        { label: '编辑', run: () => setSelectedId(note.id) },
...
function PrivateNotesPanel({ state, setState }: StatePanelProps) {
  const text = useText();
  const [selectedId, setSelectedId] = useState<string | undefined>(state.privateNotes[0]?.id);
  const [checkedIds, setCheckedIds] = useState<string[]>([]);
  ...
  const selected = state.privateNotes.find((note) => note.id === selectedId) ?? privateNotes[0] ?? state.privateNotes[0];
    ...
    {previewNote && (
      <div className="modal-backdrop" role="presentation">
        <section className="modal-panel note-preview-modal" role="dialog" aria-modal="true" aria-label={text.privateNotes.previewTitle}>
          ...
          <div className="modal-actions"><button className="toolbar-button icon-only" type="button" aria-label={text.common.pin} onClick={() => { void pinPrivateNote(previewNote); setPreviewNote(undefined); }}><Pin size={16} /></button><button className="toolbar-button" type="button" onClick={() => setPreviewNote(undefined)}>{text.common.close}</button></div>
        </section>
      </div>
    )}
    {pinnedNotes.map((pinned, index) => <PinnedNoteWindow key={pinned.id} note={pinned.note} index={index} onClose={() => setPinnedNotes((current) => current.filter((item) => item.id !== pinned.id))} />)}
    {previewImage && (
      <div className="modal-backdrop" role="presentation">
        <section className="modal-panel note-preview-modal" role="dialog" aria-modal="true" aria-label="浏览笔记图片">
          ...
          <div className="modal-actions"><button className="toolbar-button icon-only" type="button" aria-label="钉住图片" onClick={() => { void pinPreviewImage(previewImage); setPreviewImage(undefined); }}><Pin size={16} /></button><button className="toolbar-button" type="button" onClick={() => setPreviewImage(undefined)}>关闭</button></div>
        </section>
      </div>
    )}
    </>
...
function RemindersPanel({ state, setState }: StatePanelProps) {
  const [selectedId, setSelectedId] = useState<string | undefined>(state.reminders[0]?.id);
  const [draftReminder, setDraftReminder] = useState<Reminder | undefined>();
  const selected = draftReminder ?? state.reminders.find((reminder) => reminder.id === selectedId) ?? state.reminders[0];
...
function TasksPanel({ state, setState }: StatePanelProps) {
  const [selectedId, setSelectedId] = useState<string | undefined>(state.tasks[0]?.id);
  ...
  return (
    ...
    <SplitPanel
      ...
      list={<SimpleListWithContext items={visibleTasks} selectedId={selected?.id} labelOf={(task) => task.title} metaOf={renderTaskMeta} detailOf={renderTaskDetail} urgentOf={(task) => shouldHighlightLimitedTask(task)} pinnedOf={(task) => Boolean(task.highlighted)} doneOf={(task) => task.completed} onSelect={setSelectedId} contextActions={(task) => [
        { label: '编辑', run: () => setSelectedId(task.id) },
        { label: task.highlighted ? '取消置顶' : '置顶', run: () => updateTask(task.id, { highlighted: !task.highlighted }) },
        { label: '复制', run: () => copyTask(task) },
        { label: '导出', run: () => exportDoc(task.title, task.steps.map((step) => `${step.completed ? '[x]' : '[ ]'} ${step.body}`).join('\n')) },
        ...
        selected ? (
          <div className={['editor', selected.completed ? 'readonly-editor' : ''].join(' ')}>
            <div className="editor-toolbar">
              ...
              <ToolbarButton onClick={() => exportDoc(selected.title, selected.steps.map((step) => `${step.completed ? '[x]' : '[ ]'} ${step.body}`).join('\n'))}><Download size={16} />导出</ToolbarButton>
            </div>
...
interface SettingsPanelProps extends StatePanelProps {
  cloudRuntime: { password: string; passphrase: string };
  setCloudRuntime: Dispatch<SetStateAction<{ password: string; passphrase: string }>>;
  runCloudRecordSync: (silent?: boolean) => Promise<void>;
  mobileApp?: boolean;
}

function SettingsPanelV2({ state, setState, cloudRuntime, setCloudRuntime, runCloudRecordSync, mobileApp = false }: SettingsPanelProps) {
  const text = useText();
  ...
  function cloudConfig() {
    return {
      ...
      publishableKey: state.cloudSyncPublishableKey ?? '',
      email: state.cloudSyncEmail ?? '',
      password: cloudRuntime.password,
      passphrase: cloudRuntime.passphrase
    };
  }
  ...
  async function registerCloudAccount() {
    try {
      await signUpCloudAccount(cloudConfig());
      setCloudMessage('账号注册请求已提交。如 Supabase 开启了邮箱确认，请先完成邮箱确认。');
    } catch (error) {
  ...
  async function pushCloudSnapshot() {
    try {
      const result = await uploadCloudRecords(cloudConfig(), state);
      const syncedAt = result.updatedAt ?? new Date().toISOString();
      setState((current) => ({ ...current, cloudSyncLastSyncedAt: syncedAt, cloudSyncLastError: '' }));
      setCloudMessage('本机数据已逐条加密上传。');
    } catch (error) {
  ...
  async function pullCloudSnapshot() {
    if (!(await appConfirm('拉取云端逐条记录会按更新时间合并到本机，请先确认已经导出本地备份。', {
      title: '拉取云端记录',
      confirmLabel: '合并记录',
      danger: true
    }))) return;
    try {
      const result = await pullCloudRecords(cloudConfig(), state);
      const syncedAt = result.updatedAt ?? new Date().toISOString();
      setState((current) => ({
        ...result.state,
        clipboardItems: current.clipboardItems,
  ...
  return (
    <section className="panel full-panel">
      ...
      <div className="settings-grid">
        <div>
          <span>版本信息</span>
          <strong>{mobileApp ? '0.2.2 安卓同步版' : '0.2.2 桌面端supabase自动同步版本'}</strong>
        </div>
        {!mobileApp && <label>
          <span>开机自启动</span>
      ...
      <section className="sync-panel cloud-sync-settings">
        <h2>云端同步</h2>
        <p className="muted">Supabase URL 和 publishable key 只负责连接项目；同步仍需要云端账号、登录密码和加密口令。填完整后会自动逐条同步{mobileApp ? '。' : '，剪贴板集合仅保存在本机。'}</p>
        <div className="settings-grid">
          <label>
          ...
          <label>
            <span>云端账号</span>
            <input type="email" value={state.cloudSyncEmail ?? ''} onChange={(event) => setState((current) => ({ ...current, cloudSyncEmail: event.target.value }))} placeholder="邮箱账号" />
          </label>
          <label>
            <span>登录密码</span>
            <input type="password" value={cloudRuntime.password} onChange={(event) => setCloudRuntime((current) => ({ ...current, password: event.target.value }))} placeholder="Supabase 登录密码" />
          </label>
          <label>
            <span>加密口令</span>
            <input type="password" value={cloudRuntime.passphrase} onChange={(event) => setCloudRuntime((current) => ({ ...current, passphrase: event.target.value }))} placeholder="用于加密逐条记录，丢失后无法恢复" />
          </label>
        </div>
        <div className="toolbar">
          <button className="toolbar-button" type="button" onClick={() => void registerCloudAccount()}>注册账号</button>
          <button className="toolbar-button" type="button" onClick={() => void runCloudRecordSync(false).then(() => setCloudMessage('自动逐条同步已执行。'))}>立即同步</button>
          <button className="toolbar-button" type="button" onClick={() => void pushCloudSnapshot()}>仅上传逐条记录</button>
          <button className="toolbar-button danger-action" type="button" onClick={() => void pullCloudSnapshot()}>拉取逐条记录</button>
        </div>
        <div className="sync-status">
          <span>{state.cloudSyncLastSyncedAt ? `上次同步：${formatDateTime(state.cloudSyncLastSyncedAt)}` : '尚未同步'}</span>
      ...
      <section className="sync-panel">
        ...
        <div className="toolbar">
          ...
          <button className="toolbar-button" onClick={exportBackup}>导出备份</button>
...
function DatedList<T extends { id: string; createdAt: string; updatedAt: string }>({
  ...
  if (view === 'list' && contextActions) return <SimpleListWithContext items={items} selectedId={selectedId} labelOf={labelOf} metaOf={(item) => formatDateTime(item.createdAt)} urgentOf={isHighlightedItem} leadingOf={leadingOf} trailingOf={trailingOf} onSelect={onSelect} contextActions={contextActions} />;
